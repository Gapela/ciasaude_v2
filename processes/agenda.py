...
def get_df_to_json(filter=None, data=''):
    if filter==None:   
        query = """select id, pac.nome as paciente, agenda.especialidade, prof.nome as profissional, agenda.data_agenda, agenda.horario, agenda.forma_pagamento, agenda.plano,
        ...
        id = data['id_agenda']
        query = f"""select id, agenda.id_paciente, agenda.especialidade, agenda.id_profissional, agenda.data_agenda, agenda.horario, agenda.forma_pagamento, agenda.plano,
        agenda.numero_carteirinha, agenda.observacao
        from agenda as agenda
        left join paciente pac
        on agenda.id_paciente::integer = pac.id_paciente
